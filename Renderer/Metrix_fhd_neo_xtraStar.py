...
class Metrix_fhd_neo_xtraStar(VariableValue, Renderer):
    def __init__(self):
        print("renderer")
        logout(data="----------------------------------------------------------------------------- init")
    ...
    def changed(self, what):
        ...
        try:
            ...
            if event:
                ...
                rating3_json = "{}xtraEvent/infos/{}.json".format(pathLoc, evntNm)
                ...
                if os.path.exists(rating_json):
                    ...
                    if os.path.exists(rating3_json):
                        logout(data="json nicht vorhanden hole aus infos")
                        with open(rating3_json, encoding="utf-8") as f:
                            logout(data="json vorhanden open file")
                            data = json.load(f)
                            logout(data=str(data))
                            # Zugriff auf das erste Element in "results"
                            rating = data["results"][0]["vote_average"]
                            logout(data="vote_average: {}".format(rating))

                        if rating:
                            logout(data="json wert vorhanden in infos")
                            rtng = int(10 * (float(rating)))
                            logout(data=str(rtng))
                        else:
                            logout(data="json wert nicht vorhanden in infos")
                            rtng = 0

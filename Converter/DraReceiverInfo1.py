# This converter is modded by bahaa 
...
class DraReceiverInfo1(Poll, Converter):
    HDDTEMP = 0
    LOADAVG = 1
    MEMTOTAL = 2
    MEMFREE = 3
    ...
    def getText(self):
        ...
        if self.type == self.HDDTEMP:
            ...
            entry = {self.MEMTOTAL: ('Mem', 'Ram'),
             self.MEMFREE: ('Mem', 'Ram Libera'),  #by bahaa
            ...
            if list[0] == 0:
                text = '%s: Dispositivo Non Connesso' % entry[1]
            elif self.shortFormat:
                text = '%s: %s, in use: %s%%' % (entry[1], self.getSizeStr(list[0]), list[3])
            elif self.fullFormat:
                text = '%s: %s Libero:%s Uso:%s (%s%%)' % (entry[1],
                 self.getSizeStr(list[0]),
                 self.getSizeStr(list[2]),
                 self.getSizeStr(list[1]),
                 list[3])
            else:
                # below line added by bahaa please do not remove them to show total or free ram
                if self.type == self.MEMTOTAL:
                   text = '%s: %s Ram In Uso:%s Libera:%s' % (entry[1],self.getSizeStr(list[0]),self.getSizeStr(list[1]),self.getSizeStr(list[2]))    #by bahaa  Ram text
                elif self.type == self.MEMFREE:
                   text = '%s: %s' % (entry[1],self.getSizeStr(list[2]))    #by bahaa  show free ram only

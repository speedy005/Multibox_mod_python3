...
def calculateMoonPhase(year, month, day):
    """
    Berechnet die Mondphase basierend auf dem Datum.
    """
    if month < 3:
        year -= 1
        month += 12
    ...
    c = 365.25 * year
    e = 30.6 * month
    jd = c + e + day - 694039.09  # d ist die Anzahl der vergangenen Tage
    jd /= 29.5305882              # urch den Mondzyklus teilen (ca. 29,53 Tage)
    b = int(jd)                   # ganzzahligen Anteil von jd berechnen (Tagesanzahl)
    jd -= b                       # nur den Nachkommaanteil behalten (Mondphasenfortschritt)
    phase = round(jd * 8)         # in 8 Mondphasen skalieren und runden
    if phase >= 8:
        phase = 0
    phases = [
        "Neumond",
...
class Metrixfhd_neo_OAWeather(Converter, object):
	CURRENT = 0
	DAY1 = 1
	DAY2 = 2
	DAY3 = 3
	...
	def getText(self):
		if self.mode:
			try:
				...
				elif self.mode == "moonphase":
					today = datetime.datetime.now()
					return calculateMoonPhase(today.year, today.month, today.day)

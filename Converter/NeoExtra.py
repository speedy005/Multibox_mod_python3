#	BetaExtra converter
#	Modded and recoded by Neo for use Metrixfhd skins or standalone, added Python3 support
			...
			try:
				if os.path.exists("/proc/stb/sensors/temp0/value"):
					with open("/proc/stb/sensors/temp0/value") as stemp:
						systemp = "Cpu: %s°C" % stemp.readline().replace("\n", "")
				...
				if not cpuspeed:
					try:
						...
						try:
							...
							cpuspeed = "-"
				return "Velocità Cpu: %s MHz" % cpuspeed
			except:
				return ""
		...
		if self.type == self.FANINFO:
			...
			if uptime_info != None:
				...
				if self.shortFormat:
					...
					if days > "0":
						uptime += days + " " + (days == "1" and "day" or "Giorni") + ", "
					if len(uptime) > 0 or hours > "0":
						uptime += hours + " " + (hours == "1" and "hr" or "Ore") + ", "
					if len(uptime) > 0 or minutes > "0":
						uptime += minutes + " " + (minutes == "1" and "min" or "Minuti")+ ", "
					uptime += seconds + ' ' + (seconds == "1" and "Second" or "Sec")
				return "Tempo: %s" % uptime

...
from Components.MXComponents import isImageType, isPY2
import os

DBG = False
if DBG: from Components.MXComponents import MXDEBUG

class MXBitrate(Converter, object):
    ...
    @cached
    def getText(self):
        if DBG: MXDEBUG("[MXBitrate:getText] vcur %s" % self.vcur)
        if self.vcur > 0:
    ...
    def doSuspend(self, suspended):
        if DBG: MXDEBUG("[MXBitrate:suspended] >>> self.isSuspended=%s, suspended=%s" % (self.isSuspended,suspended))
        if suspended == 0:
            self.isSuspended = False
            self.StartTimer.start(100, True)
        else:
    ...
    def start(self):
        if self.isRunning == False:
            if self.source.service:
                if DBG: MXDEBUG("[MXBitrate:start] initiate runTimer")
                self.isRunning = True
                self.runTimer.start(100, True)
            else:
                if DBG: MXDEBUG("[MXBitrate:start] wait 100ms for self.source.service")
                self.StartTimer.start(100, True)
        else:
            if DBG: MXDEBUG("[MXBitrate:start] runBitrate in progress, nothing to do")

    def runBitrate(self):
        if DBG: MXDEBUG("[MXBitrate:runBitrate] >>>")
        if isImageType('vti'):
            demux = 2
        else:
            adapter = 0
        ...
        try:
            ...
            if stream:
                if DBG: MXDEBUG("[MXBitrate:runBitrate] Collecting stream data...")
            ...
            if DBG: MXDEBUG("[MXBitrate:runBitrate] Exception collecting stream data: %s" % str(e))
        try:
            info = self.source.service.info()
            vpid = info.getInfo(iServiceInformation.sVideoPID)
            apid = info.getInfo(iServiceInformation.sAudioPID)
        except Exception as e:
            if DBG: MXDEBUG("[MXBitrate:runBitrate] Exception collecting service info: %s" % str(e))
        ...
        if vpid >= 0 and apid >= 0:
            ...
            if DBG: MXDEBUG('[MXBitrate:runBitrate] starting "%s"' % cmd)
            self.container.execute(cmd)
    
    def clearValues(self, *args): #invoked by appClosed & kill from suspend
        if DBG: MXDEBUG("[MXBitrate:clearValues] >>>")
        self.isRunning = False
    ...
    def appClosed(self, retval):
        if DBG: MXDEBUG("[MXBitrate:appClosed] >>> retval=%s, isSuspended=%s" % (retval, self.isSuspended))

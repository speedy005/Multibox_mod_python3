...
class Metrixfhd_neo_iCaidInfo2(Poll, Converter, object):
    CAID = 0
    PID = 1
    PROV = 2
    ALL = 3
    ...
    def getText(self):
        ...
        if not service:
            ...
            elif not info.getInfoObject(iServiceInformation.sCAIDs): return _('Frei Empfangbar')
            elif not ecm_info:
                if self.NUM_CI and self.NUM_CI > 0:
                    CIinfo = self.getCIdata(False)
                    if CIinfo == '' and softCamName == '':
                        return _("keine daten von CI")
                    elif CIinfo == '':
                        return _("keine daten von CI und emulator")
                    else:
                        return _(CIinfo)
                else:
                    return _("kein daten vom emulator")

...
class AMB_ExtremeInfo2(Poll, Converter, object):
    TUNERINFO = 0
    CAMNAME = 1
    NUMBER = 2
    ECMINFO = 3
    ...
    def __init__(self, type):
        ...
        if type == 'TunerInfo':
            ...
            self.type = self.POWCRYPT
        elif type == 'PowEcm':
            self.type = self.POWECM
        elif type == 'TanCrypt':
            self.type = self.TANCRYPT
        elif type == 'TanEcm':
            self.type = self.TANECM
        elif type == 'Mgcamd':
    ...
    def getBoolean(self):
        ...
        if self.type == self.BISECM:
            ...
            return caemm
        if self.type == self.POWCRYPT:
            caemm = self.getPowCrypt()
            return caemm
        if self.type == self.POWECM:
            caemm = self.getPowEcm()
            return caemm
        if self.type == self.TANCRYPT:
            caemm = self.getTanCrypt()
            return caemm
        if self.type == self.TANECM:
            caemm = self.getTanEcm()
            return caemm
    ...
    def getWicardd(self):
        ...
        return False
# okay decompyling /tmp/ExtremeInfo2.pyo 
# 
# 

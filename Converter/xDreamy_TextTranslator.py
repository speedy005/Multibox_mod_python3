...
gettext.bindtextdomain("xDreamy", "%s%s" % (
    resolveFilename(SCOPE_PLUGINS), "Extensions/xDreamy/locale/"))


def _(txt):
    t = gettext.dgettext("xDreamy", txt)
    if t == txt:
        t = gettext.gettext(txt)
    return t
###########################################################################################################################################

class xDreamy_TextTranslator(Converter, object):
	movecopy = 0
	currentlyrunning = 1
	itfollows = 2
	coming = 3

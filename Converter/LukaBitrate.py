...
DBG = False
DEBUG_FILE = '/tmp/LukaComponents.log'
...
def LUKADEBUG(my_text=None, append=True, debug_file=DEBUG_FILE):
...
class LukaBitrate(Converter, object):
	...
	def getText(self):
		if DBG:
			LUKADEBUG(f"[LukaBitrate:getText] vcur {self.vcur}")
	...
	def doSuspend(self, suspended):
		if DBG:
			LUKADEBUG(f"[LukaBitrate:suspended] >>> self.is_suspended={self.is_suspended}, suspended={suspended}")
	...
	def start(self):
		if not self.is_running:
			if self.source.service:
				if DBG:
					LUKADEBUG("[LukaBitrate:start] initiate run_timer")
				self.is_running = True
				self.run_timer.start(100, True)
			else:
				if DBG:
					LUKADEBUG("[LukaBitrate:start] wait 100ms for self.source.service")
				self.start_timer.start(100, True)

	def run_bitrate(self):
		if DBG:
			LUKADEBUG("[LukaBitrate:run_bitrate] >>>")
		...
		try:
			...
			if stream:
				if DBG:
					LUKADEBUG("[LukaBitrate:run_bitrate] Collecting stream data...")
			...
			if DBG:
				LUKADEBUG(f"[LukaBitrate:run_bitrate] Exception collecting stream data: {e}")
		...
		try:
			...
			if DBG:
				LUKADEBUG(f"[LukaBitrate:run_bitrate] Exception collecting service info: {e}")
			return
		...
		if vpid >= 0 and apid >= 0:
			...
			if DBG:
				LUKADEBUG(f'[LukaBitrate:run_bitrate] starting "{cmd}"')
			self.container.execute(cmd)

	def clear_values(self, *args):
		if DBG:
			LUKADEBUG("[LukaBitrate:clear_values] >>>")
	...
	def app_closed(self, retval):
		if DBG:
			LUKADEBUG(f"[LukaBitrate:app_closed] >>> retval={retval}, is_suspended={self.is_suspended}")
	...
	def data_avail(self, data):
		if DBG:
			LUKADEBUG(f"[LukaBitrate:data_avail] >>> data '{data}'\n\tself.remaining_data='{self.remaining_data}'")

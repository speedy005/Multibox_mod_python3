...
novpnfile = '/usr/share/enigma2/Metrixfhd-neo/samba/novpn.txt'
isvpnfile = '/usr/share/enigma2/Metrixfhd-neo/samba/isvpn.txt'
    ...
    try:
        if not os.path.exists(myipdata) or time() - os.path.getmtime(r"%s" % myipdata) > myipdataupdatepause:
            if sys.version_info[0] >= 3:
                ...
                open("/tmp/VPN-Py3.txt", "w").write(ablauf)
                ...
                open("/tmp/VPN-Py2.txt", "w").write(ablauf)
...
class Metrixfhd_neo_UTIPdata(Poll, Converter, object):

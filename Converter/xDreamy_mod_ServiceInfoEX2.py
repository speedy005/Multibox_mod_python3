...
class xDreamy_mod_ServiceInfoEX2(Poll, Converter, object):
	apid = 0
	vpid = 1
	sid = 2
	onid = 3
	...
	def getText(self):
		...
		if self.type == self.apid:
			...
			streaminfo = self.stream['xres'] + 'x' + self.getServiceInfoString(info, iServiceInformation.sVideoHeight) +  ("i", "p", "")[info.getInfo(iServiceInformation.sProgressive)] + ' ' + self.stream['fps'] + ' fps'

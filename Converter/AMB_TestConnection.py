...
BYTES = 4096
test = "1234"
open("/tmp/Internet", "w").write(test)
...
class AMB_TestConnection(Converter, object):
    def __init__(self, type):
        ...
        config.misc.standbyCounter.addNotifier(
            self.enterStandby, initial_call=False)
    ...
    def get_iface_list(self):
        ...
        bytelen = struct.unpack(
            'iL',
            fcntl.ioctl(sck.fileno(), SIOCGIFCONF,
                        struct.pack('iL', BYTES,
                                    names.buffer_info()[0])))[0]
        sck.close()
        namestr = names.tostring()
        return [
            namestr[i:i + 32].split(b'\x00', 1)[0]
    ...
    @cached
    def getBoolean(self):
        open("/tmp/Internet_OK", "w").write(test)
        return self.testOK

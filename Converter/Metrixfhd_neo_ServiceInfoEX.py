...
class Metrixfhd_neo_ServiceInfoEX(Poll, Converter, object):
	apid = 0
	vpid = 1
	sid = 2
	onid = 3
	...
	def getText(self):
		...
		if self.type == self.apid:
			...
			return ["SDR", "HDR", "HDR10", "HLG"].get(gamma, "")
			...
			return encryption_map.get(caidinfo[0], "") if caidinfo else "FTA"
			...
			video_codec = codec_data.get(info.getInfo(iServiceInformation.sVideoType), "")
			...
			gamma = gamma_map.get(gamma_value, "")

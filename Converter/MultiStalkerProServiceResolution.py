...
class MultiStalkerProServiceResolution(Poll, Converter, object):
    ...
    def __init__(self, type):
        ...
        elif type == "VideoComplex":
            self.type = self.VIDEOALL
        else:
            self.type = self.ALL
        ...
        if self.type == self.VIDEOMODE:
            ...
            if width > 0 and height > 0:
                ...
                if sProgressive:
                    ...
                    text += "i" + str((info.getInfo(iServiceInformation.sFrameRate) + 499) // 500)
                codec_id = str(info.getInfo(iServiceInformation.sVideoType))
                codec = VIDEO_CODECS[codec_id]
                text += " [%s]" % codec
        else:
            print("type must be {VideoMode, VideoSize, VideoSizeWidth, VideoSizeHeight, VideoSizeShort, VideoCodec, VideoFormat} for ServiceResolution converter")
            text = "type-error"
